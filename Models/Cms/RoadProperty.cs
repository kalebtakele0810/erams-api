// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
#nullable disable
using System;
using System.Collections.Generic;

namespace ERAManagementSystem.Models
{
    public partial class RoadProperty
    {
        public int RoadPropertyId { get; set; }
        public int? EngineeringEstimateId { get; set; }
        public string SurfaceType { get; set; }
        public decimal? Width { get; set; }
        public decimal? ShoulderWidth { get; set; }
        public decimal? Length { get; set; }
        public decimal? RightParkingLaneWidth { get; set; }
        public decimal? RightWalkwayWidth { get; set; }
        public decimal? LeftParkingLaneWidth { get; set; }
        public decimal? LeftWalkwayWidth { get; set; }
        public string LastEditedBy { get; set; }
        public DateTime? CreatedOn { get; set; }
        public DateTime? LastModifiedOn { get; set; }
        public string CreatedBy { get; set; }

        public virtual EngineeringEstimate EngineeringEstimate { get; set; }
    }
}